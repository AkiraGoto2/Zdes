                    ...
                    <div class="bg-white rounded-2xl border border-amber-200 overflow-hidden">
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-center gap-3 mt-4 pt-4 border-t border-gray-100">
                                ...
                                <button type="button"
                                    onclick="openRejectModal('{{ route('admin.events.reject', $event) }}')"
...
<div id="reject-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:99999;align-items:center;justify-content:center;">
    <div style="background:white;border-radius:20px;padding:28px;max-width:440px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,.3);position:relative;">
        <h3 style="font-size:17px;font-weight:700;margin-bottom:16px;">Отклонить событие</h3>
        <form id="reject-form" method="POST">
            @csrf
            <label style="display:block;font-size:13px;font-weight:500;color:#374151;margin-bottom:6px;">Причина отклонения</label>
            <textarea name="reason" rows="4"
                style="width:100%;border:1.5px solid #e5e7eb;border-radius:12px;padding:10px 14px;font-size:13px;resize:none;outline:none;box-sizing:border-box;"
                onfocus="this.style.borderColor='#ef4444'"
                onblur="this.style.borderColor='#e5e7eb'">Не соответствует правилам площадки.</textarea>
            <div style="display:flex;gap:10px;margin-top:16px;">
                ...
                <button type="submit"
                    style="flex:1;padding:10px 0;background:#ef4444;border:none;border-radius:12px;font-size:14px;font-weight:600;color:white;cursor:pointer;">
                    Отклонить
                </button>
            </div>
        </form>
...
function openRejectModal(action) {
    ...
    document.querySelector('#reject-modal textarea').focus();
}
