...
<div class="max-w-[1280px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2">
            {{-- Кнопка фильтров (всегда видна, на больших экранах форма всегда открыта) --}}
            <button id="filter-toggle-btn"
                class="sm:hidden flex items-center gap-1.5 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-600 hover:bg-gray-50"
                onclick="document.getElementById('filter-form').classList.toggle('hidden')">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-.293.707L13 13.414V19a1 1 0 01-.553.894l-4 2A1 1 0 017 21v-7.586L3.293 6.707A1 1 0 013 6V4z"/></svg>
                Фильтры
                @if(request()->hasAny(['q','category','filter','age','date_from','date_to','time_from','time_to']))
    ...
    {{-- ФИЛЬТРЫ — на мобайле скрыты по умолчанию --}}
    <div id="filter-form" class="hidden sm:block mb-6">
    <form method="GET" action="{{ route('events') }}">
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <div class="flex gap-2 mt-4">
                ...
                @if(request()->hasAny(['q','category','filter','age','date_from','date_to','time_from','time_to','sort']))
