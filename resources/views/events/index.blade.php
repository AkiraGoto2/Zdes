...
<div class="max-w-[1280px] mx-auto px-6 py-8">
    ...
    <form method="GET" action="{{ route('events') }}" class="mb-8">
        <div class="bg-white rounded-2xl border border-gray-200 p-4 flex flex-wrap gap-4 items-end">
            ...
            <div class="flex-1 min-w-[140px]">
                <label class="block text-xs font-medium text-gray-500 mb-1">Когда</label>
                <select name="date"
                    class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#4A40E0]">
                    <option value="">Любая дата</option>
                    <option value="today" {{ request('date') === 'today' ? 'selected' : '' }}>Сегодня</option>
                    <option value="week" {{ request('date') === 'week' ? 'selected' : '' }}>На этой неделе</option>
                </select>
            </div>
            ...
            <div class="flex-1 min-w-[120px]">
                <label class="block text-xs font-medium text-gray-500 mb-1">Возраст</label>
                <select name="age"
                    class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#4A40E0]">
    ...
    @if($events->isEmpty())
        <div class="text-center py-24 text-gray-400">
            ...
            <p class="text-lg font-medium">Событий не найдено</p>
                    ...
                    <div class="p-4 flex flex-col flex-1">
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-100">
                            ...
                            @elseif($event->price_to)
                                <span class="text-sm font-semibold text-gray-800">{{ number_format($event->price, 0, '', ' ') }} – {{ number_format($event->price_to, 0, '', ' ') }} ₽</span>
                            @else
