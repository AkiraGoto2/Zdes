    ...
    <div class="max-w-2xl mx-auto px-4 py-10">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                    ...
                    <div class="bg-gray-50 rounded-xl p-3">
                        ...
                        @if($event->event_date_end)
                            <p class="text-xs text-gray-500 mt-0.5">
                                — {{ \Carbon\Carbon::parse($event->event_date_end)->translatedFormat('d F Y, H:i') }}
                            </p>
                        @endif
                    </div>
                    <div class="bg-gray-50 rounded-xl p-3">
                ...
                @if($event->contact_phone || $event->contact_site || $event->contact_telegram || $event->contact_vk)
                    <div class="mb-4">
                        <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Контакты</h2>
                        <div class="space-y-2">
                            @if($event->contact_phone)
                                <div class="flex items-center gap-2 text-sm text-gray-700">
                                    <span class="text-xs text-gray-400 w-16 flex-shrink-0">Контакт</span>
                                    @if(str_starts_with($event->contact_phone, 'http'))
                                        <a href="{{ $event->contact_phone }}" target="_blank" class="text-[#4A40E0] hover:underline">{{ $event->contact_phone }}</a>
                                    @elseif(str_contains($event->contact_phone, '@'))
                                        <a href="mailto:{{ $event->contact_phone }}" class="text-[#4A40E0] hover:underline">{{ $event->contact_phone }}</a>
                                    @elseif(str_starts_with($event->contact_phone, '@'))
                            ...
                            @endif
                            @if($event->contact_telegram)
                                <div class="flex items-center gap-2 text-sm">
                                    <span class="text-xs text-gray-400 w-16 flex-shrink-0">Telegram</span>
                                    @if(str_starts_with($event->contact_telegram, 'http'))
                                        <a href="{{ $event->contact_telegram }}" target="_blank" class="text-[#4A40E0] hover:underline">{{ $event->contact_telegram }}</a>
                                    @else
                                        <span class="text-gray-700">{{ $event->contact_telegram }}</span>
                                    @endif
                                </div>
                            @endif
                            @if($event->contact_vk)
                                <div class="flex items-center gap-2 text-sm">
                                    <span class="text-xs text-gray-400 w-16 flex-shrink-0">ВКонтакте</span>
                                    @if(str_starts_with($event->contact_vk, 'http'))
                                        <a href="{{ $event->contact_vk }}" target="_blank" class="text-[#4A40E0] hover:underline">{{ $event->contact_vk }}</a>
                                    @else
                                        <span class="text-gray-700">{{ $event->contact_vk }}</span>
                                    @endif
                                </div>
                            @endif
                            @if($event->contact_site)
                                <div class="flex items-center gap-2 text-sm">
                                    <span class="text-xs text-gray-400 w-16 flex-shrink-0">Сайт</span>
                                    <a href="{{ $event->contact_site }}" target="_blank" class="text-[#4A40E0] hover:underline truncate">{{ $event->contact_site }}</a>
                                </div>
                            @endif
                        </div>
                    </div>
        ...
        @auth
            @if(Auth::id() !== $event->user_id && $event->status === 'approved')
                ...
                @else
                    @if(is_null($event->price))
                        {{-- Бесплатное событие — просто запись --}}
                    ...
                    @else
                        {{-- Платное событие — покупка билета --}}
                        <a href="{{ route('tickets.create', $event) }}"
                            class="mt-4 block w-full text-center bg-[#4A40E0] text-white rounded-xl py-3 text-sm font-semibold hover:bg-[#3d35c7] transition-colors">
                            Купить билет
                        </a>
                    @endif
                @endif
